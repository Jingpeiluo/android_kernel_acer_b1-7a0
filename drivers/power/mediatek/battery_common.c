...
static unsigned int g_BatteryNotifyCode;
...
#define Battery_charge_max_vol 4325 
#define Battery_charge_min_vol 2600
...
static void mt_battery_notify_badbattery_check(void)
{
	if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT ||g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
	{
	       ...
	       {
	               g_BatteryNotifyCode |= 0x0020;//the new value:0x0012 is for show_charger_badbattery_logo                                          
	       }
       }
}
...
*/	
	return 1; 	//force to return 1 ,because just tcl battery be used.
