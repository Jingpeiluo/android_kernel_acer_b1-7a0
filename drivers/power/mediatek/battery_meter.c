...
struct battery_meter_custom_data my_batt_meter_cust_data;
...
int __batt_meter_init_cust_data_from_dt(void)
{
	...
	if (read_battery_id() == 1)
	{
		__batt_meter_parse_table(np, "r_profile_t0_tcl",
	...
	if (read_battery_id() == 1)
	{
		__batt_meter_parse_table(np, "r_profile_t2_tcl",
	...
	if (read_battery_id() == 1)
	{
		__batt_meter_parse_table(np, "r_profile_t3_tcl",
...
signed int g_Q_MAX_POS_50[] = {2500,2500};
signed int g_Q_MAX_POS_25[] = {2550,2550 };
signed int g_Q_MAX_POS_0[] = {2500,2500};
signed int g_Q_MAX_NEG_10[] = {2500,2500};
signed int g_Q_MAX_POS_50_H_CURRENT[] = {2500,2500};
signed int g_Q_MAX_POS_25_H_CURRENT[] = {2550,2550};
signed int g_Q_MAX_POS_0_H_CURRENT[] = {2500,2500};
signed int g_Q_MAX_NEG_10_H_CURRENT[] = {2400,2400};
...
void oam_init(void)
{
	...
	/*Neostra huangxiaohui add for RTC capacity  Bug 143173  Bug 141853 20170213*/
	signed int g_rtc_soc = 0;
	signed int grtc_voltage = 0;  
    ...
    /*Neostra huangxiaohui add for RTC capacity Bug 143173  Bug 141853 20170213*/
    g_rtc_soc = mtk_misc_get_spare_fg_value();
    grtc_voltage = fgauge_read_v_by_d((100-g_rtc_soc));
	...
	gFG_capacity_by_v_init = gFG_capacity_by_v;
	    //add start Neosta huangxiaohui add use rtc capacity Bug 143173  Bug 141853 20170215
    if(bat_is_charger_exist() == KAL_TRUE)
    {
        if((g_rtc_soc != 0) && (abs(gFG_voltage - grtc_voltage)>60 ))
        {            
            gFG_voltage = grtc_voltage;
			gFG_capacity_by_v = g_rtc_soc;
            gFG_capacity_by_v_init = gFG_capacity_by_v;
            printk("===hxh=== use rtc voltage as: gFG_voltage=%d,gFG_capacity_by_v=%d\n",grtc_voltage,gFG_capacity_by_v);
        }
...
/*--- add by lwj for test ---*/
	printk("lwj[oam_init_inf] %d, %d, %d, %d, %d, %d\n",
		 gFG_voltage, (100 - fgauge_read_capacity_by_v(gFG_voltage)), g_rtc_fg_soc,
		 gFG_DOD0, oam_v_ocv_init, force_get_tbat(KAL_FALSE));
	printk("lwj[oam_run_inf] %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
		 oam_v_ocv_1, oam_v_ocv_2, vol_bat, oam_i_1, oam_i_2, oam_r_1, oam_r_2, oam_car_1,
		 oam_car_2, gFG_BATT_CAPACITY_aging, force_get_tbat(KAL_FALSE), oam_d0);

	printk("lwj[oam_result_inf] %d, %d, %d, %d, %d, %d\n",
		 oam_d_1, oam_d_2, oam_d_3, oam_d_4, oam_d_5, BMT_status.UI_SOC);
...
{	
	...
	BATTERY_PROFILE_STRUCT *temp_profile_p;	
	R_PROFILE_STRUCT  *temp_profile_r_p;	
	int i, saddles;	
	
	temp_profile_p=fgauge_get_profile(my_batt_meter_cust_data.temperature_t);
	...
	for (i = 0; i < saddles; i++) {		
		printk("bat_pro_t: <DOD,Voltage> at %d = <%d,%d>\r\n",
			my_batt_meter_cust_data.temperature_t, (temp_profile_p + i)->percentage,
			(temp_profile_p + i)->voltage);	
	}	
	
	temp_profile_r_p = fgauge_get_profile_r_table(my_batt_meter_cust_data.temperature_t);	/* Dumpt new r-table profile */	
	for (i = 0; i < saddles; i++) {		
		printk("r_pro_t:<Rbat,VBAT> at %d = <%d,%d>\r\n",
			my_batt_meter_cust_data.temperature_t, (temp_profile_r_p + i)->resistance,
...
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
	 if(r_t_flag==0)
		temp_profile_p=&battery_profile_t0[g_fg_battery_id][0];
	else if(r_t_flag==1)
		temp_profile_p=&battery_profile_t1[g_fg_battery_id][0];
	else if(r_t_flag==2)
		temp_profile_p=&battery_profile_t2[g_fg_battery_id][0];
	else if(r_t_flag==3)
		temp_profile_p=&battery_profile_t3[g_fg_battery_id][0];
	else
		temp_profile_p=&battery_profile_t2[g_fg_battery_id][0];
#else
	 if(r_t_flag==0)
		temp_profile_p=&battery_profile_t0[0];
	else if(r_t_flag==1)
		temp_profile_p=&battery_profile_t1[0];
	else if(r_t_flag==2)
		temp_profile_p=&battery_profile_t2[0];
	else if(r_t_flag==3)
		temp_profile_p=&battery_profile_t3[0];
	else
		temp_profile_p=&battery_profile_t2[0];
#endif
...
{
	R_PROFILE_STRUCT  *temp_profile_p;
...
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
	 if(r_t_flag==0)
		temp_profile_p=&r_profile_t0[g_fg_battery_id][0];
	else if(r_t_flag==1)
		temp_profile_p=&r_profile_t1[g_fg_battery_id][0];
	else if(r_t_flag==2)
		temp_profile_p=&r_profile_t2[g_fg_battery_id][0];
	else if(r_t_flag==3)
		temp_profile_p=&r_profile_t3[g_fg_battery_id][0];
	else
		temp_profile_p=&r_profile_t2[g_fg_battery_id][0];
#else
	 if(r_t_flag==0)
		temp_profile_p=&r_profile_t0[0];
	else if(r_t_flag==1)
		temp_profile_p=&r_profile_t1[0];
	else if(r_t_flag==2)
		temp_profile_p=&r_profile_t2[0];
	else if(r_t_flag==3)
		temp_profile_p=&r_profile_t3[0];
	else
		temp_profile_p=&r_profile_t2[0];
	...
	ptr +=sprintf(ptr, "r_pro_t%d:<Rbat,VBAT> at %d \r\n", r_t_flag,
			 my_batt_meter_cust_data.temperature_t);
		
	/* Dumpt new r-table profile */
	for (i = 0; i < saddles; i++) {
		printk("r_pro_t%d:<Rbat,VBAT> at %d = <%d,%d>\r\n",r_t_flag,
			 my_batt_meter_cust_data.temperature_t, (temp_profile_p + i)->resistance,
			 (temp_profile_p + i)->voltage);
...
static ssize_t show_bat_profile_val(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	temp_profile_p=&battery_profile_temperature[0];
...
static ssize_t show_r_profile_val(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	temp_profile_p=&r_profile_temperature[0];
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	my_batt_meter_cust_data=batt_meter_cust_data; //add by lwj for test
