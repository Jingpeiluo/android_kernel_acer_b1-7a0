...
//ALPS03255445--there is no respond on pc when during copying files to sd, then plug out sd
	...
	while (count > 0 || write_req) {
		if (count > 0) {
			...
			if (ret < 0) {
				...
				//dev->state = STATE_ERROR;
				//ALPS03255445--there is no respond on pc when during copying files to sd, then plug out sd
				ERROR(cdev, "%s, line %d: EIO, dev->dev_disconnected = %d, usb queue error \n", __func__, __LINE__, dev->dev_disconnected);
		...
		if (write_req) {
			...
			if (ret != write_req->actual) {
				...
				//dev->state = STATE_ERROR;
				//ALPS03255445--there is no respond on pc when during copying files to sd, then plug out sd
				ERROR(cdev, "%s, line %d: EIO, dev->dev_disconnected = %d, file write error \n", __func__, __LINE__, dev->dev_disconnected);
