...
static void dump_op_state(void)
{
	...
	for (i = 0; i < 4; i++)
		DBG(5, "0x%x = 0x%x\n", 0x640 + i * 4, musb_readl(mtk_musb->mregs, 0x640 + i * 4));
}
#endif

static void dump_phy_reg(u8 reg)
{
	u8 value = USBPHY_READ8(reg);
...
{
	DBG(5, "Dump PHY Registers Begin\n");
	dump_phy_reg(0x1a);
	dump_phy_reg(0x1d);
	...
	DBG(5, "Dump PHY Registers End\n");
}

static void musb_force_utmi(struct musb *musb)
{
#ifdef MUSB_ENABLE_BUS_SUSPEND
...
#ifdef MUSB_ENABLE_BUS_SUSPEND
	if (mtk_musb->is_host) {
		utmi0 = USBPHY_READ32(0x68);
		DBG(5, "PHY 0x68-6B:0x%x\n", utmi0);
		utmi1 = USBPHY_READ32(0x6C);
		...
		reg32 = musb_readl(musb_base, MUSB_HSDMA_INTR);
		DBG(5, "MUSB_HSDMA_INTR:0x%x\n", reg32);
	}
#endif

}

static void musb_force_host_release_utmi(struct musb *musb)
{
#ifdef MUSB_ENABLE_BUS_SUSPEND
	u8 devctl;
	u8 power, testmode;
	...
	devctl = musb_readb(mtk_musb->mregs, MUSB_DEVCTL);
	DBG(5, "devctl:0x%x\n", devctl);
	testmode = musb_readb(mtk_musb->mregs, MUSB_TESTMODE);

	/* Reg:0x610 = 0x5 */
	musb_writel(mtk_musb->mregs, 0x610, 0x5);	/* add by chengchun */

	DBG(5, "testmode:0x%x\n", testmode);
	if (speed == 2) {	/* HS */
		musb_writeb(mtk_musb->mregs, MUSB_TESTMODE, 0x90);
		musb_writeb(mtk_musb->mregs, MUSB_DEVCTL,
			    musb_readb(mtk_musb->mregs, MUSB_DEVCTL) | 0x01);

		dump_op_state();
		DBG(5, "Before force SE0\n");

		/* Force SE0 */
		reg32 = USBPHY_READ32(0x80);
		reg32 &= (0xFFFFFFFC);
		reg32 |= (1 << 2);	/* Bit2: maybe means FORCE_LINESTATE */
		USBPHY_WRITE32(0x80, reg32);

		/* suspend bus */
		power |= (MUSB_POWER_SUSPENDM | MUSB_POWER_ENSUSPEND);
		musb_writeb(mtk_musb->mregs, MUSB_POWER, power);
		DBG(5, "after suspend bus, power:0x%x\n", power);
		mdelay(15);

		dump_op_state();

		reg32 = musb_readl(mtk_musb->mregs, 0x630);
		DBG(5, "0x630:0x%x\n", reg32);
		...
		if (speed == 1) {	/* FS */
			musb_writeb(mtk_musb->mregs, MUSB_TESTMODE, 0xA0);
		} else if (speed == 0) {	/* LS */
			musb_writeb(mtk_musb->mregs, MUSB_TESTMODE, 0x80);
		}
		/* Session Bit: set 1 */
		musb_writeb(mtk_musb->mregs, MUSB_DEVCTL, musb_readb(mtk_musb->mregs, MUSB_DEVCTL) | 0x01);
		...
		reg32 = musb_readl(mtk_musb->mregs, 0x620);
		DBG(5, " 0x620:0x%x\n", reg32);
	...
	/* Release UTMI */
	reg32 = USBPHY_READ32(0x68);
	reg32 &= ~((1 << FORCE_TERMSEL) | (1 << FORCE_TXVALID)
		   | (1 << FORCE_DPPULLDOWN) | (1 << FORCE_DMPULLDOWN)
		   | (1 << FORCE_XCVRSEL) | (1 << FORCE_OPMODE)
		   | (1 << FORCE_SUSPENDM));
	reg32 |= (1 << FORCE_SUSPENDM) | (1 << RG_SUSPENDM);
	USBPHY_WRITE32(0x68, reg32);

	DBG(5, "After release UTMI\n");
	dump_op_state();

	musb_writeb(mtk_musb->mregs, MUSB_TESTMODE, 0x00);	/*Clear Test Mode */
	DBG(5, "0x630 = %x\n", musb_readl(mtk_musb->mregs, 0x630));
#endif
}
...
static void musb_save_context(struct musb *musb)
{
	...
	musb->context.ep0_func_addr = musb_readb(musb_base, MUSB_TXFUNCADDR);
	musb->context.power = power;
	musb->context.intrusbe = musb_readb(musb_base, MUSB_INTRUSBE);
	musb->context.index = musb_readb(musb_base, MUSB_INDEX);
	musb->context.devctl = devctl;

	musb->context.l1_int = musb_readl(musb_base, USB_L1INTM);
	DBG(5, "TXFUNCADDR:0x%x, MUSB_INTRUSBE:0x%x, INDEX:0x%x\n",
	    musb->context.ep0_func_addr, musb->context.intrusbe, musb->context.index);

#ifdef MUSB_ENABLE_BUS_SUSPEND
	if (mtk_musb->is_host) {
		speed = (power & MUSB_POWER_HSMODE) ? 2 : -1;
		if (speed == -1) {
		...
		musb->context.speed = speed;
		DBG(4, "musb speed   mode:%s\n",
		    speed == 2 ? ("HS") : (speed == 1 ? "FS" : "LS"));
	}
#endif

	DBG(5, "musb->config->num_eps:%d\n", musb->config->num_eps);
	for (i = 0; i < musb->config->num_eps; ++i) {
		...
		if (!hw_ep)
			continue;
		DBG(5, "i:%d, epnum:%d, ep_mode:%d\n", i, hw_ep->epnum, hw_ep->ep_mode);
		epio = hw_ep->regs;
		if (!epio)
			continue;
...
#ifdef MUSB_ENABLE_BUS_SUSPEND
		musb->context.index_regs[i].txfunaddr = musb_readb(epio, MUSB_TXFUNCADDR + 8 * i);
		musb->context.index_regs[i].txhubaddr = musb_readb(epio, MUSB_TXHUBADDR + 8 * i);
		musb->context.index_regs[i].rxfunaddr = musb_readb(epio, MUSB_RXFUNCADDR + 8 * i);
		musb->context.index_regs[i].rxhubaddr = musb_readb(epio, MUSB_RXHUBADDR + 8 * i);
		DBG(5, "txfuncaddr:0x%x, txhubaddr:0x%x,rxfuncaddr:0x%x,rxhubaddr:0x%x\n",
		    musb->context.index_regs[i].txfunaddr, musb->context.index_regs[i].txhubaddr,
		    musb->context.index_regs[i].rxfunaddr, musb->context.index_regs[i].rxhubaddr);
#endif
		if (musb->dyn_fifo) {
...
static void musb_restore_context(struct musb *musb)
{
...
#ifdef MUSB_ENABLE_BUS_SUSPEND
	/* bubble set */
	reg = musb_readb(mtk_musb->mregs, 0x74);
	/* DBG(5, "read 0x74 reg:0x%x\n", reg); */
	reg &= 0x3F;
	musb_writeb(mtk_musb->mregs, 0x74, reg);
	...
	DBG(5,
	    ...
	    musb->context.ep0_func_addr, musb->context.power, musb->context.intrusbe,
	    musb->context.index, musb->context.devctl, musb->context.l1_int);
	...
	musb_writeb(musb_base, MUSB_TXFUNCADDR, musb->context.ep0_func_addr);

	devctl = musb_readb(musb_base, MUSB_DEVCTL);
	/* we don't restore DEVCTL:Bit0 Session */
	musb_writeb(musb_base, MUSB_DEVCTL, ((musb->context.devctl) & 0xFE) | (devctl & 0x01));
	...
	for (i = 0; i < musb->config->num_eps; ++i) {
		...
		musb_writeb(epio, MUSB_TXFUNCADDR + 8 * i, musb->context.index_regs[i].txfunaddr);
		musb_writeb(epio, MUSB_TXHUBADDR + 8 * i, musb->context.index_regs[i].txhubaddr);
		musb_writeb(epio, MUSB_RXFUNCADDR + 8 * i, musb->context.index_regs[i].rxfunaddr);
		musb_writeb(epio, MUSB_RXHUBADDR + 8 * i, musb->context.index_regs[i].rxhubaddr);
		DBG(5, "txfuncaddr:0x%x, txhubaddr:0x%x,rxfuncaddr:0x%x,rxhubaddr:0x%x\n",
		    musb->context.index_regs[i].txfunaddr, musb->context.index_regs[i].txhubaddr,
		    musb->context.index_regs[i].rxfunaddr, musb->context.index_regs[i].rxhubaddr);
	...
	reg32 = musb_readl(musb_base, USB_L1INTM);
#ifdef CONFIG_MTK_MUSB_QMU_SUPPORT
	...
	reg32 = musb_readl(musb_base, MUSB_HSDMA_INTR);
	DBG(5, "before, MUSB_HSDMA_INTR:0x%x\n", reg32);
	musb_writel(musb->mregs, MUSB_HSDMA_INTR, 0xFF | (0xFF << DMA_INTR_UNMASK_SET_OFFSET));
	musb_writeb(musb_base, MUSB_INTRUSBE, MUSB_INTR_CONNECT | MUSB_INTR_BABBLE
		    | MUSB_INTR_SUSPEND | MUSB_INTR_SESSREQ
		    | MUSB_INTR_VBUSERROR | MUSB_INTR_DISCONNECT | MUSB_INTR_RESUME);
	reg32 = musb_readl(musb_base, MUSB_HSDMA_INTR);
	DBG(5, "after set, MUSB_HSDMA_INTR:0x%x\n", reg32);
	reg = musb_readb(musb_base, MUSB_INTRUSBE);
	DBG(5, "after set, MUSB_INTRUSBE:0x%x\n", reg);

	DBG(5, "reg index:%d, store index:%d\n", reg, musb->context.index);
	/* Restore MAC End */
...
static int musb_resume_noirq(struct device *dev)
{
	...
	musb_force_host_release_utmi(musb);
	...
	if (!mtk_musb->is_host) {
		...
		usb_enable_clock(false);
		mtk_usb_power = false;	/* If remove, device mode: after suspend, may can't connect to Host. */
#ifdef CONFIG_MTK_MUSB_PORT0_LOWPOWER_MODE
		mt_usb_clock_unprepare();
#endif
	}
