...
//Neostra ldk add for charger leds 0826
static unsigned int red_led_gpio;
static unsigned int green_led_gpio;
//Neostra ldk add for charger leds 0826
...
//Neostra liudongke add for charger leds 0826
...
static void musb_id_pin_work(struct work_struct *data)
{
	...
	if (mtk_musb->is_host) {
		/*Neostra liudongke add for shutdown charger leds when is host mode start*/
		//mt65xx_leds_brightness_set(MT65XX_LED_TYPE_RED, 0);
		//mt65xx_leds_brightness_set(MT65XX_LED_TYPE_GREEN, 0);
		led_set_gpio_output(red_led_gpio,0);
		led_set_gpio_output(green_led_gpio,0);
		/*Neostra liudongke add for shutdown charger leds when is host mode end*/
...
void mt_usb_otg_init(struct musb *musb)
{
	...
	//Neostra ldk add for charger led 0826
	node = of_find_compatible_node(NULL, NULL, "mediatek,red");
	if (node == NULL) {
		printk("led red get node failed\n");
	}
	else
	{
	   red_led_gpio = of_get_named_gpio(node, "led_gpios_red", 0);
	}
	node = of_find_compatible_node(NULL, NULL, "mediatek,green");
	if (node == NULL) {
		printk("led green get node failed\n");
	}
	else
	{
	   green_led_gpio = of_get_named_gpio(node, "led_gpios_green", 0);
	}
	//Neostra ldk add for charger led 0826
#if !defined(CONFIG_MTK_LEGACY)
