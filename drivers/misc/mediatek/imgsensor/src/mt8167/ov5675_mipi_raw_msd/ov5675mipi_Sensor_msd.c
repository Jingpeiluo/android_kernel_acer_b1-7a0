...
#include "ov5675mipi_Sensor_msd.h"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV5675_MSD_MIPI_SENSOR_ID,	/* record sensor id defined in Kd_imgsensor.h */
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) + 1;
			if (*sensor_id == imgsensor_info.sensor_id) { //id: 0x5676
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				printk("[hjl] get_imgsensor_id: found ov5675[sp5506] msd module.\n");
				spin_lock(&imgsensor_drv_lock);
				gIsMsdModule = 1;
				spin_unlock(&imgsensor_drv_lock);
				return ERROR_NONE;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) + 1;  //Neostra huangjianlong 20170717
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
...
{
	LOG_INF("scenario_id = %d\n", scenario_id);
	spin_lock(&imgsensor_drv_lock); //mtk add 20170425
	switch (scenario_id) {
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
	...
	spin_unlock(&imgsensor_drv_lock);//mtk add 20170425
	return ERROR_NONE;
}
...
UINT32 OV5675_MIPI_RAW_SensorInit_msd(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
